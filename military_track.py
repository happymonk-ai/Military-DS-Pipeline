...
def run(
        ...
        device_id = device_id,
        queue1 = Queue(),
        yolo_weights=WEIGHTS / 'three_class_05_dec.pt',  # model.pt path(s),
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        # Inference
        visualize = increment_path(save_dir / Path(path[0]).stem, mkdir=False) if visualize else False
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_vid:
                ...
                # cv2.imshow('Detections', im0) # to save detected image
